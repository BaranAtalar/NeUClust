...
class NeuralUCBDiag:
    ...
    def get_ucb_cluster(self, context):
...
#         self.len += 1
#         optimizer = optim.SGD(self.func_super.parameters(), lr=1e-3, weight_decay=self.lamdba / self.len)
...
for t in range(min(args.size, b.size)):
    ...
    g_list_clusters = []
    indices = []
    max_vals_clusters = np.zeros((len(X_prefs_clusters),K))
    for n in range(len(X_prefs_clusters)):
        arms_of_cluster = available_arms[n]
        cluster_base_rewards = []
        #context_arms_cluster = np.zeros((len(X_prefs_clusters[n]),20))
        cluster_context = []
        for i in range(len(arms_of_cluster)):
            context_arm = arms_of_cluster[i].context
            # predicted_rew_base = l.func(torch.from_numpy(np.transpose(context_arm.reshape(-1,1))).to(device='cuda', dtype=torch.float32))
            # cluster_base_rewards.append(float(predicted_rew_base))
            cluster_context.append(context_arm)
        
        #top_K_cluster_rew_predicted = (np.array(cluster_base_rewards))[xx]
        #top_K_cluster_context = (np.array(cluster_context))[xx]
        
        #(torch.tensor(cluster_base_rewards).size())
        #predicted_rew_super = lm.func_super(torch.tensor(top_K_cluster_rew_predicted).to(device='cuda', dtype=torch.float32))
        #print(len(cluster_context))
        ucbs,g_list_cluster = l.get_ucb_cluster(list(cluster_context))
        g_list_clusters.append(g_list_cluster)
        ucb_cluster.append(ucbs)
        xx = np.argpartition((ucbs.cpu()).detach().numpy(),-K)[-K:]
        indices.append(xx)
        max_vals_clusters[n,:] = ((ucbs.cpu().detach()).numpy())[xx]
        
        
        #avg_ucb_cluster.append(torch.mean(ucb_cluster[n]).item())
    ind_vals_arr = np.argwhere(np.isin(max_vals_clusters, np.sort(max_vals_clusters, axis=None)[-K:]))
    final_arms = []
    best_g_list = []
    for el in ind_vals_arr:
        clust = el[0]
        inddd = el[1]
        final_arms.append(available_arms[clust][indices[clust][inddd]])
        best_g_list.append(g_list_clusters[clust][indices[clust][inddd]])
    #best_cluster_index = avg_ucb_cluster.index(max(avg_ucb_cluster))
    #best_cluster_g_list = g_list_clusters[best_cluster_index]
    updating = l.update_params(best_g_list)
    selected_cluster_arms = final_arms
