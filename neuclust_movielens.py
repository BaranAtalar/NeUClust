...
X = np.zeros((len(user_ids),20))
cnt = 0
for idd in user_ids:
    X[cnt,:] = user_id_prefs_dict[idd]
    cnt += 1
...
for n in range(n_clusters):
    #n_indices = np.where(labels_clusters == n)[0]
    #print(len(n_indices))
    X_prefs_clusters.append(X[np.where(labels_clusters == n)[0],:])
...
class Bandit_multi:
    ...
    def step(self,t):
        ...
        for j in range(len(X_prefs_clusters)):
            aaa = np.zeros((len(X_prefs_clusters[j]),20))
            for i in range(len(X_prefs_clusters[j])):
                aaa[i] = self.X_prefs_clusters[j][i]*X_movie
            X_final.append(aaa)
        return X_final
...
class Network(nn.Module):
    def __init__(self, dim = 20, hidden_size=50):
...
class NeuralUCBDiag:
    def __init__(self, style, K, dim, num_users, lamdba=1, nu=1, hidden=50):
...
class NetworkMono(nn.Module):
    def __init__(self, dim = 5, hidden_size=25):
        ...
        # )
        #nn.Sequential(nn.Linear(dim, hidden_size),nn.ReLU(),nn.Linear(hidden_size, 1))
...
class NeuralUCBDiagMono:
    ...
    def train(self, rewards, super_reward):
        ...
        for _ in range(40):
            ...
            loss.backward()
            optimizer.step()
            for p in self.func_super.parameters():
                p.data.clamp_(0)
            # for p in self.func_super.parameters():
            #     print(p)
        return 0
...
#budget = 5
l = NeuralUCBDiag(args.style, K, b.dim,len(user_id_prefs_dict) , args.lamdba, args.nu, args.hidden)
lm = NeuralUCBDiagMono(K,args.lamdba, args.nu, 20)
...
for t in range(min(args.size, b.size)):
    ...
    for n, arms_of_cluster in enumerate(available_arms):
        ...
        cluster_contexts_torch = torch.from_numpy(cluster_contexts.reshape(-1, 20)).to(device='cuda', dtype=torch.float32)
